...
struct Table<K, V, S, R> {
    ...
    buckets: Vec<RwLock<R, Bucket<K, V>>>,
}

impl<K, V, S: Default, R: RawRwLock> Table<K, V, S, R> {
...
impl<K, V, S: BuildHasher, R: RawRwLock> Table<K, V, S, R> {
...
impl<K: PartialEq + Hash, V, S: BuildHasher, R: RawRwLock> Table<K, V, S, R> {
    ...
    fn scan<F, Q: ?Sized>(&self, key: &Q, matches: F) -> StableRwLockReadGuard<R, Bucket<K, V>>
    where
        ...
        Q: Hash,
    {
        ...
        for i in 0..self.buckets.len() {
            ...
            if matches(&lock) {
                // Yup. Return.
                return StableRwLockReadGuard(lock);
            }
        }
    ...
    /// returned.
    fn scan_mut<F, Q: ?Sized>(&self, key: &Q, matches: F) -> StableRwLockWriteGuard<R, Bucket<K, V>>
    where
        F: Fn(&Bucket<K, V>) -> bool,
        K: Borrow<Q>,
        Q: Hash,
    {
        ...
        for i in 0..self.buckets.len() {
            ...
            if matches(&lock) {
                // Yup. Return.
                return StableRwLockWriteGuard(lock);
            }
        }
    ...
    fn lookup_or_free(&self, key: &K) -> RwLockWriteGuard<R, Bucket<K, V>> {
        // Hash the key.
        let hash = self.hash(key);
    ...
    fn lookup<Q: ?Sized>(&self, key: &Q) -> StableRwLockReadGuard<R, Bucket<K, V>>
    where
        K: Borrow<Q>,
        Q: PartialEq + Hash,
    {
    ...
    fn lookup_mut<Q: ?Sized>(&self, key: &Q) -> StableRwLockWriteGuard<R, Bucket<K, V>>
    where
        K: Borrow<Q>,
        Q: PartialEq + Hash,
    {
    ...
    /// inserted.
    fn find_free(&self, key: &K) -> StableRwLockWriteGuard<R, Bucket<K, V>> {
...
impl<K: Clone, V: Clone, S: Clone, R: Clone + RawRwLock> Clone for Table<K, V, S, R> {
    fn clone(&self) -> Self {
        Table {
...
impl<K: fmt::Debug, V: fmt::Debug, S, R: RawRwLock> fmt::Debug for Table<K, V, S, R> {
...
pub struct IntoIter<K, V, S, R> {
    /// The inner table.
    table: Table<K, V, S, R>,
}

impl<K, V, S, R: RawRwLock> Iterator for IntoIter<K, V, S, R> {
    type Item = (K, V);
...
impl<K, V, S, R: RawRwLock> IntoIterator for Table<K, V, S, R> {
    type Item = (K, V);
    type IntoIter = IntoIter<K, V, S, R>;
...
impl<'a, K, V, S, R: RawRwLock> ops::Deref for ReadGuard<'a, K, V, S, R> {
    type Target = V;
...
impl<'a, K, V, S, R: RawRwLock> ops::DerefMut for WriteGuard<'a, K, V, S, R> {
...
pub struct CHashMap<K, V, S, R> {
    /// The inner table.
    table: RwLock<R, Table<K, V, S, R>>,
...
impl<K, V, S: Default, R: RawRwLock> CHashMap<K, V, S, R> {
...
impl<K, V, S, R: RawRwLock> CHashMap<K, V, S, R> {
...
impl<K, V, S: Default + BuildHasher, R: RawRwLock> CHashMap<K, V, S, R> {
    ...
    S: Default + BuildHasher,
{
    ...
    pub fn clear(&self) -> CHashMap<K, V, S, R> {
...
impl<K: PartialEq + Hash, V, S: BuildHasher, R: RawRwLock> CHashMap<K, V, S, R> {
    ...
    /// is held.
    pub fn get<Q: ?Sized>(&self, key: &Q) -> Option<ReadGuard<K, V, S, R>>
    where
        K: Borrow<Q>,
        Q: Hash + PartialEq,
    {
    ...
    /// is held.
    pub fn get_mut<Q: ?Sized>(&self, key: &Q) -> Option<WriteGuard<K, V, S, R>>
    where
        K: Borrow<Q>,
        Q: Hash + PartialEq,
    {
    ...
    R: RawRwLock
{
    ...
    fn expand(&self, lock: RwLockReadGuard<R, Table<K, V, S, R>>) {
...
impl<K, V, S: Default + BuildHasher, R: RawRwLock> Default for CHashMap<K, V, S, R> {
    fn default() -> Self {
...
impl<K: fmt::Debug, V: fmt::Debug, S, R: RawRwLock> fmt::Debug for CHashMap<K, V, S, R> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        (*self.table.read()).fmt(f)
    }
}

impl<K, V, S, R: RawRwLock> IntoIterator for CHashMap<K, V, S, R> {
    type Item = (K, V);
    type IntoIter = IntoIter<K, V, S, R>;

    fn into_iter(self) -> IntoIter<K, V, S, R> {
        self.table.into_inner().into_iter()
    }
}

impl<K: PartialEq + Hash, V, S: Default + BuildHasher, R: RawRwLock> iter::FromIterator<(K, V)>
    for CHashMap<K, V, S, R>
{
